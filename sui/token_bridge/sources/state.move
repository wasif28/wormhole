module token_bridge::state {
    ...
    const E_UNREGISTERED_TOKEN: u64 = 3;
    ...
    public(friend) fun take_from_circulation<CoinType>(
        ...
        if (registered_tokens::is_wrapped<CoinType>(registry)) {
            registered_tokens::burn(registry, coin);
        } else {
            registered_tokens::deposit(registry, coin);
        }
    }
    ...
    public fun take_from_circulation_test_only<CoinType>(
        ...
        take_from_circulation(self, coin)
    }
    ...
    public fun put_into_circulation_test_only<CoinType>(
        ...
        put_into_circulation(self, amount, ctx)
    }
    ...
    /// Retrieve immutable reference to `TreasuryCap` for a wrapped coin.
    /// Currently this `TreasuryCap` is only used to give the capability to
    /// update `CoinMetadata`.
    ///
    /// NOTE: There is no danger of this reference being used to mint coins
    ...
    public(friend) fun treasury_cap<CoinType>(
        self: &State
    ): &TreasuryCap<CoinType> {
        registered_tokens::treasury_cap(&self.registered_tokens)
    }
...
module token_bridge::bridge_state_test {
    ...
    fun test_coin_type_addressing_(test: Scenario) {
        ...
        next_tx(&mut test, admin); {
            ...
            let (token_chain, token_address) = state::token_info<NATIVE_COIN_10_DECIMALS>(&bridge_state);
