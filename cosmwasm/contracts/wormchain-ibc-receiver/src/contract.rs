...
use crate::msg::{ChainConnectionResponse, ExecuteMsg, QueryMsg};
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        QueryMsg::ChainConnection(chain) => {
            query_chain_connection(deps, chain).and_then(|resp| to_binary(&resp))
        }
    }
}

fn query_chain_connection(deps: Deps, chain: u16) -> StdResult<ChainConnectionResponse> {
    CHAIN_CONNECTIONS
        .load(deps.storage, chain)
        .map(|connection_id| ChainConnectionResponse { connection_id })
}
